...
import { fetchSignInMethodsForEmail , signInWithPopup , GoogleAuthProvider , setPersistence , browserLocalPersistence } from "firebase/auth";
...
const GoogleLoginUp = () => {
    ...
    const handleGoogleAuth = async () => {
        try{
            ...
            console.error("GoogleLoginUp Error:", error.message);
            alert("An error occurred during Google login.");
        }
    }

    return (
        <button onClick={handleGoogleAuth} type="button" className="mt-3 inline-flex w-full  items-center justify-center rounded-md text-white bg-black/50 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
